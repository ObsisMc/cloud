...
- **Downstream consumer**: `frontend/src/api` is generated by orval from `api/openapi.json`. After a contract change run `task frontend:generate` (= `task openapi` + frontend generation) and commit both; the CI `frontend` job fails on frontend drift.

See [cmd overview](../README.en.md), [Contract package](../../internal/contract/README.en.md), [HTTP router](../../internal/api/router/README.en.md), and [Web frontend](../../frontend/README.en.md).
