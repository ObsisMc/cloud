...
- With `[github]` filled in, writes `client_secret` to `.local/gateway/github-client-secret` (the file the Gateway's `github.client_secret_file` points at); with it empty, removes any stale secret file and the Gateway offers only the development login.
- Writes `.local/dev.env` (`-env` overrides): `CLOUD_DATABASE_DSN`, `GATEWAY_DATABASE_DSN` and `TEST_DATABASE_URL` from `[database].dsn`, `GATEWAY_GITHUB_CLIENT_ID` from `[github].client_id`. `Taskfile.yml` loads it through `dotenv` for every task, so `task dev`, `task migrate`, `task test` and `task check` share one set of values; variables already set in the shell take precedence.
...
- **Strict parsing**: an unknown key in `config.toml` fails with its location; `database.dsn` is required, and `github.client_id` / `github.client_secret` are set together or left empty together.
