...
- **Key loading**: Reads two PKCS#8 Ed25519 private keys (separate service and user purposes), the PKCE derivation key, and the GitHub client secret from files into process memory only.
- **Boundary assembly**: Builds the `gateway.Store`, GitHub adapter, `gateway.Login`, `gateway.Issuer`, rate limiter, and `gateway.NewHandler`, then listens on the configured port.
