...
- **密钥装载**：从文件读取两把 PKCS#8 Ed25519 私钥（service 与 user 用途分离）、PKCE 派生密钥与 GitHub client secret，只保存在进程内存。
- **边界装配**：构造 `gateway.Store`、GitHub 适配器、`gateway.Login`、`gateway.Issuer`、限流器与 `gateway.NewHandler`，监听配置端口。
