...
`internal/gateway` 实现 Gateway 的认证编排、PostgreSQL 会话存储、内部凭据签发、浏览器安全策略与 Cloud 代理。它是 provider-neutral 的：只有 `Authenticator` 适配器（如 [github](github/README.md) 与仅限本地开发的 [devlogin](devlogin/README.md)）理解外部协议，其余部分只消费 `VerifiedIdentity` 与已解析的 `Session`。
...
| `login.go` | `Login`：`Start` 生成 attempt secret、`state`，派生 PKCE verifier（HMAC，密钥仅 Gateway 可读）；`Callback` 先查 attempt，在事务外调用 provider，再消费 attempt 创建 session。所有失败收敛为 `ErrLoginFailed`。`Providers` 列出已注册的适配器；`CodeChallenge` 是导出的 S256 变换。 |
...
参见 [内部子系统总览](../README.md)、[GitHub 适配器](github/README.md)、[本地开发 provider](devlogin/README.md) 与 [Gateway 文档](../../docs/gateway.md)。
