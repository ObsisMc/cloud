...
func isList(r router.Route) bool {
	return r.Method == "GET" && (strings.HasSuffix(r.Path, "/tenants") || strings.HasSuffix(r.Path, "/members") || strings.HasSuffix(r.Path, "/projects") || strings.HasSuffix(r.Path, "/workspaces") || strings.HasSuffix(r.Path, "/spaces") || strings.HasSuffix(r.Path, "/resource-status") || strings.HasSuffix(r.Path, "/clones"))
}
...
func description(r router.Route) string {
	...
	if strings.Contains(r.Path, "/spaces") {
		...
		case r.Method == "POST" && strings.HasSuffix(r.Path, "/spaces"):
			base += "Creates the workspace and its first owner atomically. slug is lowercase, immutable and unique per tenant. "
		case strings.Contains(r.Path, "/projects"):
			base += "Project collection scoped to one collaboration workspace; membership is required. "
		case r.Method == "PATCH":
			base += "Only name and description may change; slug is immutable. Requires admin or owner and a matching version. "
		case r.Method == "DELETE":
			base += "Archives the workspace (soft delete); requires owner and a matching version. Projects are unaffected. "
		default:
