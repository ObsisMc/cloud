...
`task frontend:*` needs Node 24 with npm >= 11.19 (the version CI's `npm ci` uses; `engine-strict` refuses
older ones so the lock file is never regenerated by an npm that drops entries) and `npm ci` in `frontend/`. The Go gates do not depend on it, and
